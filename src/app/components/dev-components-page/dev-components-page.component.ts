...
export class DevComponentsPageComponent {
  ...
  openExampleDialog(): void {
    ...
    dialogRef.beforeClosed().subscribe(result => result === 'action' && this.toastService.showSuccessToast($localize`Dialog action dispatched.`));
  }

  showSuccessToast(): void {
    this.toastService.showSuccessToast($localize`Short success message (5 seconds).`);
  }

  showErrorToast(): void {
    this.toastService.showErrorToast($localize`Short error message (7 seconds).`);
  }

  showInfoToast(): void {
    this.toastService.showInfoToast($localize`Short info message (custom 9 seconds).`, { duration: 9000 });
  }
}
