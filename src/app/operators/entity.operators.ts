...
import { selectEntityById } from '../helpers/entity.helpers';
...
export function getEntityByRouterParam<T>(store: Store, entityMap$: Observable<Dictionary<T>>, param: keyof RouteParams): Observable<T | undefined> {
    return combineLatest([entityMap$, store.select(selectRouterParams).pipe(map(params => params?.[param]))]).pipe(map(selectEntityById));
}
